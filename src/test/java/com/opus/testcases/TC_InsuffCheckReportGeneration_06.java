...
public class TC_InsuffCheckReportGeneration_06 extends BaseClass {
	// Verify Approval TL Role by Admin
	@Test(enabled = true, testName = "Verify Approval TL Role by Admin", priority = 1)
	...
	// Verify Opus Approver Role by Admin 
	@Test(enabled = true, testName = "Verify Opus Approver Role by Admin ", priority = 2, dependsOnMethods = {
			"verifyAdminAprovalTl" })
	public void verifyadminApproverRole() throws InterruptedException, IOException {
		RaiseInsufficiency insuffApprover = new RaiseInsufficiency(driver);
		String data = new String(Files.readAllBytes(Paths.get("BridgeCaseArsNo.txt")));
		String arsNumber[] = data.split(" ");
		insuffApprover.adminApproverRole(arsNumber[6], arsNumber[6]);
		logger.info("Admin has selected Approver role done successfully!");
		if (insuffApprover.verifyCheckCompletedText().contains("Completed")) {
			Assert.assertTrue(true);
	...
	// Verify Reports Manager -- Search Report
